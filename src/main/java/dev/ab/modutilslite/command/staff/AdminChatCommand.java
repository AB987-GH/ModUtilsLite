...
public class AdminChatCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if (!(Perms.has(player, "modutils.command.adminchat"))){
            player.sendMessage(Chat.format("&cYou do not have permission to use this command."));
            return false;
        }

        if (args.length == 0){
            player.sendMessage(Chat.format("&eCommand usage: /ac &f<message>"));
            return false;
        }

        String message = main.getConfig().getString("messages.admin-chat");
        ...
        if (main.isUsingRedis()){
            main.getRedisManager().publish("adminchat", message);
        } else {
