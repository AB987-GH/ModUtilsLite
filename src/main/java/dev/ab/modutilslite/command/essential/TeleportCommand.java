...
public class TeleportCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if (!(Perms.has(player, "modutils.command.teleport"))){
            player.sendMessage(Chat.format("&cYou do not have permission to use this command!"));
            return false;
        }

        if (args.length != 1){
            player.sendMessage(Chat.format("&cUsage: /tp <player>"));
            return false;
        }
        ...
        String msg = main.getConfig().getString("messages.teleport");
        String alertMsg = main.getConfig().getString("messages.teleport-alert");
        ...
        for (Player p : Bukkit.getOnlinePlayers()){
            if (Perms.has(p, "modutils.alerts.teleport")){
